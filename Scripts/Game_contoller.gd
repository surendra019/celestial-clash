...
func _ready():
	admob.initialize()
	admob.initialization_completed.connect(func(data):
		print("init completed")
		admob.load_banner_ad()
		
		)
	admob.banner_ad_loaded.connect(func(id):
			print("banner loaded")
			admob.show_banner_ad()
			
			)
	admob.rewarded_ad_loaded.connect(func(id):
		admob.show_rewarded_ad()
		)
	#admob.rewarded_ad_user_earned_reward.connect(func(id, data):
		#admob.load_banner_ad()
		#)
	#await admob.initialization_co	mpleted
...
func _on_admob_rewarded_ad_user_earned_reward(ad_id, reward_data):
	admob.load_banner_ad()
	Manager.set_panel(game_over, "hide", self)
	player.player_health.value = 100
	game_over.btn2.disabled = false
